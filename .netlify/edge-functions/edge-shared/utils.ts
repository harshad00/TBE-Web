...
import {
  ElementHandlers,
  HTMLRewriter,
} from 'https://deno.land/x/html_rewriter@v0.1.0-pre.17/index.ts';
...
export function updateModifiedHeaders(
  requestHeaders: Headers,
  responseHeaders: Headers
) {
  const overriddenHeaders = responseHeaders.get(
  ...
  const headersToUpdate = overriddenHeaders
    .split(',')
...
export const buildNextRequest = (
  ...
  const { country, subdivision, city, latitude, longitude, timezone } =
    context.geo;
...
export const buildResponse = async ({
  ...
  if (isMiddlewareResponse(result.response)) {
    ...
    if (
      response.originResponse.headers
        .get('content-type')
    ...
    if (response.elementHandlers.length > 0) {
      response.elementHandlers.forEach(([selector, handlers]) =>
